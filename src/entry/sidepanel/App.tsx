...
const App: React.FC = () => {
    ...
    const handleSummarizeResponseStream = (data: SummarizeResponse) => {
        ...
        if (data.done) {
            ...
            if (data.content && aiGenerationAnalyzerRef.current) {
                aiGenerationAnalyzerRef.current.reset()
                aiGenerationAnalyzerRef.current.inputStream(data.content)
            }
            parsingStateRef.current = {
                currentBuffer: '',
                state: ParsingState.FREE,
    ...
    const handleAssistantResponseStream = (data: any) => {
        ...
        if (data.done && data.content) {
            if (data.data && aiGenerationAnalyzerRef.current) {
                aiGenerationAnalyzerRef.current.inputStream(data.data)
            } else if (aiGenerationAnalyzerRef.current) {
                aiGenerationAnalyzerRef.current.inputStream(data.content)
            }
            return
        }
        if (data.content && !data.thinking && aiGenerationAnalyzerRef.current) {
            // 使用aiGenerationAnalyzer处理流式内容
            aiGenerationAnalyzerRef.current.inputStream(data.content)
        }
    }
